...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '⚫️ربات در این سوپرگروه فعال شد⚫️'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end

--Check Members #غیرفعال supergroup
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '⚪️ربات در این سوپرگروه غیرفعال شد⚪️'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end

--Function to فعال supergroup
...
--Function to غیرفعال supergroup
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "برکناری ادمین" then
		local user_id = result.from.peer_id
		local channel_id = "channel#id"..result.to.peer_id
	...
	elseif get_cmd == "تنظیم مدیر" then
	...
	elseif get_cmd == "برکناری مدیر" then
		local full_name = (result.from.first_name or '')..' '..(result.from.last_name or '')
		local member_name = full_name:gsub("‮", "")
		local member_username = member_name:gsub("_", " ")
    if result.from.username then
...
local function run(msg, matches)
	...
	elseif msg.to.type == 'channel'then
		if matches[1] == '🆙' then
			...
			return "❕اینجا درحال حاضر یک سوپرگروه میباشد❕"
		end
	end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'فعال' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '⚫️ربات از قبل فعال شده است⚫️', ok_cb, false)
			end
		...
		if matches[1] == 'غیرفعال' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '⚪️ربات فعال نشده است⚪️', ok_cb, false)
			end
			print("ربات "..msg.to.print_name.."("..msg.to.id..") دراین سوپرگروه غیرفعال شد")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "ابتدا لینک جدیدی ایجاد یا ذخیره کنید،برای ذخیر از دستور (ثبت لینک) استفاده کنید"
			end
		...
		if matches[1] == 'برکناری ادمین' then
			...
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'برکناری ادمین',
					msg = msg
				}
				demoteadmin = get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'برکناری ادمین' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'برکناری ادمین'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'برکناری ادمین' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'برکناری ادمین'
				}
		...
		if matches[1] == 'تنظیم مالک' and is_owner(msg) then
			...
			local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					...
					local text = "[ "..matches[2].." ] کاربر به عنوان مالک گروه انتخواب شد"
					return text
				end
		...
		if matches[1] == 'تنظیم مدیر' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == 'تنظیم مدیر' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'تنظیم مدیر'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'تنظیم مدیر' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'تنظیم مدیر',
				}
		...
		if matches[1] == 'برکناری مدیر' then
			if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
			...
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'برکناری مدیر',
					msg = msg
				}
				demote = get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'برکناری مدیر' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'برکناری مدیر'
		...
		if matches[1] == "ثبت اسم" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "ثبت توضیحات" and is_momod(msg) then
			local receiver = get_receiver(msg)
			...
			return "توضیحات سوپرگروه ثبت شد"
		end

		if matches[1] == "ثبت یوزرنیم" and is_admin1(msg) then
		...
		if matches[1] == 'ثبت قوانین' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if msg.media then
			if msg.media.type == 'تصویر' and data[tostring(msg.to.id)]['settings']['set_photo'] == 'waiting' and is_momod(msg) then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set new SuperGroup photo")
				load_photo(msg.id, set_supergroup_photo, msg)
				return
			end
		end
		if matches[1] == 'ثبت عکس' and is_momod(msg) then
		...
		if matches[1] == 'عمومی بودن' and is_momod(msg) then
			local target = msg.to.id
